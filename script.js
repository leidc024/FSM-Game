...
function addDropDownContents() {
  ...
  dropdownMenu.innerHTML = `
      <li class="dropdown-item">
          <button type="button" class="btn btn-sm" onclick="clearAllQuestions()">Clear All Questions</button>
      </li>
  `;

  if (!regexArr.length) {
      dropdownMenu.innerHTML += '<li class="dropdown-item"><p>No Regex Uploaded</p></li>';
      return;
  }
  ...
  for (let i = 0; i < regexArr.length; i++) {
      dropdownMenu.innerHTML += `
      <li class="dropdown-item">
          ${regexArr[i]} <button type="button" class="btn btn-sm" onclick="removeRegex('${regexArr[i]}')">
...
// Load regexArr from localStorage on page load
document.addEventListener("DOMContentLoaded", function() {
  const storedRegexArr = localStorage.getItem('regexArr');
  if (storedRegexArr) {
      regexArr = JSON.parse(storedRegexArr);
  } else {
      regexArr = [];
  }
  addDropDownContents();
